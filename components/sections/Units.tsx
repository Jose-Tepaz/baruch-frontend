...
export default function Units({ units }: { units?: Unit[] }) {
  ...
  if (!units || !Array.isArray(units) || units.length === 0) {
    return <div className="units-section"></div>;
  }
            ...
            <div className="units-table-wrapper">
              ...
              <div className="table-responsive d-none d-lg-block">
                <table className="table table-hover units-table">
                  <thead>
                    <tr>
                      <th className="text-size-smaill text-weight-medium">
                        {t("propertyDetails.units.id-house")}
                      </th>
                      <th className="text-size-smaill text-weight-medium">
                        {t("propertyDetails.units.dorms")}
                      </th>
                      <th className="text-size-smaill text-weight-medium">
                        {t("propertyDetails.units.bathrooms")}
                      </th>
                      <th className="text-size-smaill text-weight-medium">
                        {t("propertyDetails.units.built-area")}
                      </th>
                      <th className="text-size-smaill text-weight-medium">
                        {t("propertyDetails.units.exterior-area")}
                      </th>
                      <th className="text-size-smaill text-weight-medium">
                        {t("propertyDetails.units.garage")}
                      </th>
                      <th className="text-size-smaill text-weight-medium">
                        {t("propertyDetails.units.storage_room")}
                      </th>
                      <th className="text-size-smaill text-weight-medium">
                        {t("propertyDetails.units.price")}
                      </th>
                      <th className="text-size-smaill text-weight-medium">
                  ...
                  <tbody>
                    {units.map((unit) => (
                      <tr key={unit.id}>
                        ...
                        <td className="bedrooms-cell">{unit.bedrooms}</td>
                        <td className="bathrooms-cell">{unit.bathrooms}</td>
                        ...
                        <td className="garage-cell">
                          <span
                            className={`badge ${unit.garage ? "badge-available" : "badge-unavailable"}`}
                          >
                            {unit.garage
                              ? t("propertyDetails.units.is_available")
                              : t("propertyDetails.units.is_not_available")}
                          </span>
                        </td>
                        <td className="storage-cell">
                          <span
                            className={`badge ${unit.storage_room ? "badge-available" : "badge-unavailable"}`}
                          >
                            {unit.storage_room
                              ? t("propertyDetails.units.is_available")
                              : t("propertyDetails.units.is_not_available")}
                        ...
                        <td className="price-cell">
                          <strong>
                            €{" "}
                            {unit.price
                              ? Number(unit.price).toLocaleString("de-DE")
                              : ""}
                          </strong>
                        </td>
