...
export default function PropertyList1({ properties }: { properties: any[] }) {
  ...
  const publicProperties = properties.filter((property) => {
    const isPrivate =
      property.is_private === true || property.isPrivate === true;
    if (process.env.NODE_ENV === "development") {
      console.log(
        `Filtering property "${property.title}": is_private=${property.is_private}, isPrivate=${property.isPrivate}, shouldFilter=${isPrivate}`,
      );
    }
    return !isPrivate;
  });
